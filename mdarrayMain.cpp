...
int main(int argc, char *argv[]) {
  /* Test program: compute the Laplacian on a grid. Grid size is input on the
     command line as DIM integers > 2. Length of the domain in the first
     dimesnion is 1. */
  ...
  for (int i = 0; i < domain.sizeOf(); i++) {
    ...
    for (int dir = 0; dir < DIM; dir++) {
      LOfPhi[i] +=
          (phi.indexShift(tuple, dir, 1) + phi.indexShift(tuple, dir, -1));
    }
